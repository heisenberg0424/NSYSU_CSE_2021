...
int TCP::file(string filename){
    ...
    int file = open(filepath.c_str(),O_WRONLY|O_TRUNC|O_CREAT,S_IRWXU);

    strcpy(sendPkt.data,filename.c_str());
    sendPkt.mode = FILE;
    sendto(fd,&sendPkt,sizeof(sendPkt),0,(struct sockaddr*) &srv,sizeof(srv));
    recvfrom(fd,&recvPkt,sizeof(recvPkt),0,(struct sockaddr*) &srv,&srvlen);
    cout<<"File size: "<<recvPkt.ackNum<<endl;
    size = recvPkt.ackNum;
    ...
    while(size>0){
        for(int i=0;i<2;i++){   //Delayed ack
            recvfrom(fd,&recvPkt,sizeof(recvPkt),0,(struct sockaddr*) &srv,&srvlen);
            cout<<"     Received pkt from server (seq: "<<recvPkt.seqNum<<" )"<<endl;
            if(recvPkt.flag[CHECKSUM]==1){
                cout<<"!!!!! Invalid packet received !!!!!"<<endl;
            }
            else if(recvPkt.seqNum-offset==ack){
                if(size<=MSS){
                    test(write(file,recvPkt.data,size),"write file");
                    sendPkt.flag[FIN]=1;
                    if(i==0){
                        //useless packet
                        recvfrom(fd,&recvPkt,sizeof(recvPkt),0,(struct sockaddr*) &srv,&srvlen);
...
int main(){
    ...
    cout<<"Welcome to udp client, enter command(s):"<<endl;
    client.help();
    while(1){
        cout<<"[UDP@client]: ";
        flag=1;
